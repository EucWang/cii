...
int kref_getword(char *line, int *i, char *first, char *rest) {
	int j;
...
int kref_compare(const void *x, const void *y) {
	return Str_cmp(*(char **)x, 1, 0, *(char **)y, 1, 0);
}

unsigned kref_strhash(const void *x) {
	const char *str = x;
	unsigned h = 0;
...
void kref_print(Table_T files) {
	int i;

	void **array = Table_toArray(files, NULL);
	qsort(array, Table_length(files), 2*sizeof (*array), kref_compare);
	for (i = 0; array[i]; i += 2) {
		Seq_T seq = array[i+1];
...
void kref(char *name, FILE *fp, Table_T identifiers) {
	...
	for (linenum = 1; fgets(buf, sizeof buf, fp) != NULL; linenum++) {
		int i, j;
		for (i = 0; (j = kref_getword(buf, &i, first, rest)) > 0; i = j) {
			...
			if (files == NULL) {
				files = Table_new(0,
					(int (*)(const void *, const void *))strcmp, kref_strhash);
...
int kref_main(int argc, char *argv[]) {
	int i;
		...
		kref(NULL, stdin, identifiers);
	{
		...
		qsort(array, Table_length(identifiers), 2*sizeof (*array), kref_compare);
		for (i = 0; array[i]; i += 2) {
			Fmt_print("%S", array[i], 1, 0);
            kref_print(array[i+1]);
			FREE(array[i]);
		}
		FREE(array);
