...
int mpcalc_main(int argc, char *argv[]) {
	int c;
	sp = Seq_new(0);
	...
	while ((c = getchar()) != EOF) {
		...
		TRY
			...
			case '+': y = mpcalc_pop(); x = mpcalc_pop();
			...
			case '!': z = mpcalc_pop(); MP_not(z, z); break;
			case '~': z = mpcalc_pop(); MP_neg(z, z); break;
			case 'i': case 'o': {
				long n;
				x = mpcalc_pop();
				n = MP_toint(x);
			...
			case 'p':
				Fmt_print(f->fmt, z = mpcalc_pop(), obase);
				break;
			case 'f': {
				int n = Seq_length(sp);
				while (--n >= 0)
