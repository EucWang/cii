...
Text_T iref_getword(Text_T *line, Text_T first, Text_T rest) {
	int i, j;
...
int iref_textcmp(const void *x, const void *y) {
	return Text_cmp(*(Text_T *)x, *(Text_T *)y);
}

int iref_compare(const void *x, const void *y) {
	return iref_textcmp(*(Text_T **)x, *(Text_T **)y);
}

unsigned iref_texthash(const void *x) {
	int i;
	unsigned h = 0;
	const Text_T *t = x;
...
void iref_print(Table_T files) {
	int i;

	void **array = Table_toArray(files, NULL);
	qsort(array, Table_length(files), 2*sizeof (*array), iref_compare);
	for (i = 0; array[i]; i += 2) {
		int j;
...
void iref(char *name, FILE *fp, Table_T identifiers) {
	...
	while (fgets(buf, sizeof buf, fp) != NULL) {
		...
		while ((id = iref_getword(&line, first, rest)).len > 0) {
			Array_T array;
			Table_T files;
			files = Table_get(identifiers, &id);
			if (files == NULL) {
				files = Table_new(0, iref_textcmp, iref_texthash);
				Table_put(identifiers, iref_copy(id), files);
			}
			array = Table_get(files, &filename);
			if (array == NULL) {
				array = Array_new(1, 3);
				Table_put(files, iref_copy(filename), array);
...
int iref_main(int argc, char *argv[]) {
	int i;
	Table_T identifiers = Table_new(10000, iref_textcmp, iref_texthash);
		...
		iref(NULL, stdin, identifiers);
	{
		...
		qsort(array, Table_length(identifiers), 2*sizeof (*array), iref_compare);
		for (i = 0; array[i]; i += 2) {
			Fmt_print("%T", array[i]);
            iref_print(array[i+1]);
			FREE(array[i]);
		}
		FREE(array);
