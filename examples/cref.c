...
Text_T cref_getword(Text_T *line, Text_T first, Text_T rest) {
	int i, j;
...
int cref_textcmp(const void *x, const void *y) {
	return Text_cmp(*(Text_T *)x, *(Text_T *)y);
}

int cref_compare(const void *x, const void *y) {
	return cref_textcmp(*(Text_T **)x, *(Text_T **)y);
}

unsigned cref_texthash(const void *x) {
	int i;
	unsigned h = 0;
	const Text_T *t = x;
...
void cref_print(Table_T files) {
	int i;

	void **array = Table_toArray(files, NULL);
	qsort(array, Table_length(files), 2*sizeof (*array), cref_compare);
	for (i = 0; array[i]; i += 2) {
...
void cref(char *name, FILE *fp, Table_T identifiers) {
	...
	for (linenum = 1; fgets(buf, sizeof buf, fp) != NULL; linenum++) {
		...
		while ((id = cref_getword(&line, first, rest)).len > 0) {
			Ring_T ring;
			Table_T files;
			files = Table_get(identifiers, &id);
			if (files == NULL) {
				files = Table_new(0, cref_textcmp, cref_texthash);
				Table_put(identifiers, cref_copy(id), files);
			}
			ring = Table_get(files, &filename);
			if (ring == NULL) {
				ring = Ring_new();
				Table_put(files, cref_copy(filename), ring);
...
int cref_main(int argc, char *argv[]) {
	int i;
	Table_T identifiers = Table_new(10000, cref_textcmp, cref_texthash);
		...
		cref(NULL, stdin, identifiers);
	{
		...
		qsort(array, Table_length(identifiers), 2*sizeof (*array), cref_compare);
		for (i = 0; array[i]; i += 2) {
			Fmt_print("%T", array[i]);
            cref_print(array[i+1]);
			FREE(array[i]);
		}
		FREE(array);
